...
This controller aims to estimate the optimal target speed of the car, both in straight parts and curves of the track, taking into account two criteria: moving as fast as possible and be safe. 
...
Figure \ref {fig:cromosome} illustrates the structure of the chromosome.
%with  :
...
Tournament based selection has been used to elect chromosomes as parents for genetic operators, while simple arithmetic two point crossover is chosen with non uniform mutation \cite{GAs_Goldberg89}.
...
% Antonio - Mohammed: which kind of race? which track, which conditions, which  
%                     rivals?
...
This section is dedicated to the performance evaluation of our fuzzy-genetic controller, called \textit{FGC}.
	...
	TORCS provides several tracks and cars to choose between. In our case, we chose the E-Track5 circuit for its multiple turns and the \textit{car1-tbr1} car of SCR 1 Server\cite{evo17_blind}.\\
	% 
...
\begin{itemize}
	...
	\item[AD]: Fuzzy controller \cite{evo17_blind} .
\end{itemize}
%
Genetic algorithms are used to optimize the parameters of the fuzzy controller developed in \cite{evo17_blind} with the parameters in Table \ref{tab:parametre}:
