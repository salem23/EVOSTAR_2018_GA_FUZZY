...
This work presents an evolutionary approach to optimize the parameters of a Fuzzy-based autonomous driver for the open simulated car racing game (TORCS). Using evolution, %Pablo: "using evolution" is correct? Sounds like using evolution of the species by Darwin :P Maybe "using evolutionary algorithms" instead?
we intend to improve a modular fuzzy agent designed to determine the optimal target speed and steering angle during the race. The challenge in this kind of fuzzy systems is the design of the membership functions, which is usually done through a trial and error process.
In this paper, an adapted real-coded Genetic Algorithm - with two different fitness functions - %Pablo: two fitness functions _compared_ ?
has been applied to find the best values for these parameters, obtaining a robust design for the TORCS controller.
The evolved drivers were tested and evaluated competing against other TORCS controllers in practice (without rivals) and real races. %Pablo: "in practice mode" instead "in practice" to clarify
...
autonomous agent. Thus, this is a real enhancement of the starting fuzzy-controllers which had several difficulties to drive in this kind of circuits. %Pablo: and in future could help...
\end{abstract}
...
constraint that no accidents should take place.  %Pablo: some reference in this paragraph?
...
%Pablo: previous 3 paragraphs are too short. I think it is better to combine them in one.
...
Thus, in this work we consider the selection of the best values for these parameters as an optimization problem, so we have applied an evolutionary algorithm to obtain them. Concretely, we propose to apply a real-coded Genetic Algorithm (GA) \cite{realcodedGA1998} with this purpose. %Pablo: what the GA optimizes exactly? The MF? Say it here.
 The considered approach requires, in addition to a good codification of solutions and selection of operators, the choice of an adequate cost function (fitness), due to the uncertainty and noisiness of the problem itself. So two different proposals have been studied. %Pablo: mention here some difference between the 2 fitness
...
% FUZZY + EAs
Our approach is focused in this line, proposing the application of an off-line genetic algorithm for the improvement of the parameters which determine the behaviour of a controller for TORCS. We have focused on a Fuzzy-based model, as it is one of the best options for modelling human-like decisions and actions, as others authors have also used this kind of technique in the literature with good results. %Pablo: cite?
...
Its high degree of modularity and portability, together with the realistic and real-time driving simulation, make it an ideal testbed for artificial intelligence research, as it can be seen in the literature. %Pablo: refer to previous section?
...
Every TORCS driver bot is controlled by means of a set of actuators \cite{Torcs3}: the steering wheel `Steer', the accelerator `accel', the brake pedal and the gearbox.  %Pablo: use more large paragraphs instead short phrases.
...
\section{Simulation Results} %Pablo: I would remove "Simulation", because it also included the run of the EA, that is not a simulation.
	\label{sec:results}
...
\subsection{Simulation settings} %Pablo: same here. Better "Experimental Settings"
...
\end{table}
 ...
 The second fitness has given  higher Top Speed values  which minimize the Laptime and makes the controller operates at higher performances. Also, implying late breaking in turns which maximize the Top Speed. %Pablo: I rewrote the last phrase check if it is correct
...
In this work, we presented a Genetic Algorithm implementation to optimize and improve AD, an existing fuzzy driver for TORCS simulator  \cite{evo17_blind}, which combines two sub-controllers, one to calculate the target speed and the other for the direction (steer).
