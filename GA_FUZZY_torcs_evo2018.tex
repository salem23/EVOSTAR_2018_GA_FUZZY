...
For instance, in \cite{Guadarrama2008}, a fuzzy rule-based car controller for a Car Racing Competition was built and tuned with co-evolutionary genetic algorithms. Two fuzzy controllers were designed (acceleration and turning angle). But this approach was applied to a simpler simulator than TORCS which is more complicated and time-constrained simulator. 
...
Onieva et al.,\cite{LFAG} presented a parametrized modular architecture with a fuzzy system and a GA in the design of fuzzy logic controllers for steering wheel management that can reproduce human driver behaviour but it did not take the target speed in account unlike our previous controller which compute the target speed and the steer with two fuzzy sub-controllers and whose membership functions parameters were defined by trial/error process. 
% Say something about how we are going to improve on that - JJ. Pablo: and link with the next paragraph
% Mohammed done
In this paper, we propose to optimize these parameters using a real coded genetic algorithm in aim to improve the performances of our previous fuzzy controller.
...
Tournament based selection has been used to select chromosomes as parents for genetic operators, while simple arithmetic two point crossover and non uniform mutation \cite{GAs_Goldberg89} have been chosen.
...
\begin{description}
	...
	f_{1} =  Min & damage + &\alpha \dot Min & LapTime 
	\end{array}
	\end{equation}
	\item[Fitness 2:] 
	...
	f_{2}= Min & damage + &\alpha \dot Min & LapTime + & Min & \beta \frac{1}{TopSpeed}
	\end{array}
	\end{equation}	
\end{description}

Being $\alpha$ and $\beta$ two weighting parameters to prioritize the importance of the different objectives.
To evaluate the candidate controllers during the evolutionary process, we will make each of them compete in a 20 laps practice race in a medium difficulty circuit without  rivals.
...
Then, the obtained output values ($damage$, $LapTime$ and $TopSpeed$) are collected to compute the corresponding fitness value.  $LapTime$  is the mean of $20$ laps time
...
\begin{table}[h]
	\centering\small
	\caption{ Results of 10 runs of GA with the two fitness functions}
	{
		\begin{tabular}{|c||c|c|c||c|c|c|c|}
			\hline
			Run &\multicolumn{3}{c|}{Fitness $f_1$}&\multicolumn{4}{c|}{Fitness $f_2$}\\
			\cline{2-8}
			&Min $f_1$&$LapTime$&$Damage$&Min $f_1$&$LapTime$&$Damage$&$TopSpeed$\\
			\hline\hline
			1 &46.42& 33.42& 13&30.9843 & 30.98& 0& 234\\
			
			2 &31.09& 31.09& 0& 40.8736& 29.87& 11& 278\\
			
			3 &29.44& 29.44& 0&29.2535 & 29.25& 0& 286\\
			
			
			4 &31.12& 31.12& 0& 47.6443& 31.64& 16& 233\\
			
			5 &39.63& 31.63&8&  30.0539& 30.05& 0& 257\\
			
			6 &30.21& 30.21& 0&30.1439 & 30.14& 0& 256\\
			
			7 &29.73& 29.73& 0&   29.1237&29.12& 0& 268\\
			
			8 &37.29& 31.29&6&30.7534 & 30.75& 0& 297\\
			
			9 &34.07& 30.07& 4&29.9936 &29.99& 0& 276\\
			
			10&29.88& 29.88& 0& 29.6335&29.63& 0& 288\\
			\hline
		\end{tabular}
	}\label{tab:runsresults}
\end{table}
 The two best solution obtained  with each fitness function from these runs have been considered to be tested.  %Mohammed - ( Table shows 10 from 20 )  to reduce space 
we represented the resulted membership functions of this two optimal individuals considering the different fitness functions in Figures \ref{fig:mffront}, \ref{fig:mfmax5} and \ref{fig:MFMAX10}.
...
The shapes of the obtained memberships are completely different from those obtained by Trial/Error in the previous work \cite{evo17_blind} where the Medium linguistic variable of the new functions has bigger range.
...
\begin{table}[!ht]
	...
	\label{resultat20}
	\begin{tabular}{|c|p{3 cm}|p{2cm}|p{2cm}}
		\hline
