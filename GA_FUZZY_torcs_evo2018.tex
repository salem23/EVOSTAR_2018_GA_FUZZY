...
The evolved drivers were tested and evaluated competing against other TORCS controllers in practice and real races.
...
autonomous agent. Thus, this is a real enhancing of the starting fuzzy-controllers which had several difficulties to drive in this kind of tracks.
% Antonio - Ihave rewritten this. Mohammed, check if this is true. ;)

%Is this true? If this is explained in next sections, do not forget to add a reference to validate it.
...
point (or tick) during a race \cite{evo17_blind}.  
...
The results show that the proposed approach can successfully evolve the fuzzy controller membership parameters giving good performances in terms of damage avoidance and race time. Thus, this suggest that genetic algorithms with the proposed fitness are well-suited for finding the best trade-off between the two objectives of any racing controller: damage and speed.
% Antonio - What about the improvement with regard the previous controller? 
%           I mean, are the evolutionary obtained controllers better than the 
%           original one (the controller presented in our previous paper)?
