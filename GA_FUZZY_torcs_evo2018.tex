...
Thus, in this work we consider the selection of the best values for these parameters as an optimisation problem, so we have applied an evolutionary algorithm to obtain them. Concretely, we propose to apply a real-coded Genetic Algorithm (GA) \cite{realcodedGA1998} 
...
parameterized modular architecture with a fuzzy system
...
In this paper, we propose to optimise the membership functions parameters  of two fuzzy controllers that compute the steer and the target speed of the car in the track for TORCS. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  TORCS  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
We notice also that the GFC controller has run with a higher topspeed then GFC1. This is due to to the inclusion of Topspeed in the fitness confirming that GFC2 is the best controller.
