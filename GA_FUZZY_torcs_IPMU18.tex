...
Games offer a suitable testbed where new methodolgies and algorithms can be tested in a near-real life
...
fine-tuning the evolutionary algorithm %Pablo: Algorithm or algorithms? Are the parameters tuned?
...
usually performed using game simulators. fFor instance, The Open Racing Car Simulator
...
Onieva et al.,\cite{LFAG} presented a parametrized modular architecture with a fuzzy system and a GA in the design of fuzzy logic controllers for steering wheel management that can reproduce human driver behavior, but it did not take the target speed into account, unlike our previous controller \cite{evo17} which computed the target speed and the steer with two fuzzy sub-controllers and whose membership functions parameters were defined by trial/error process. %Pablo: why adding speed is better?
...
in the use of the $TopSpeed$ in the fitness.
