...
\urldef{\mailsc}\path| pablo.garciasanche@uca.es|
...
Games offer a suitable testbed where new methodologies and algorithms can be tested in a near-real life
environment. For example, if the game is related
...
game TORCS. The evolutionary algorithms %Pablo: How many? Why different?
change the fuzzy systems so that they set an optimal target speed as well as
...
However, the main issue with this kind of algorithms is to define a proper fitness
function that best delivers the obtained result, which is eventually %Pablo: "delivers the obtained result"? Better "models the desired result"
to win as many races as possible. In this paper we prove that
fine-tuning the controller via evolutionary algorithm %Pablo: Algorithm or algorithms? Are the parameters tuned?
robustly finds good results and
that, in some cases, they are able to play very competitively against
other published results, %Pablo: our previous work?
with a more relying approach that needs
...
difficult for any 
autonomous agent. %The Open source car racing simulation software (TORCS) is used to test the goodness of the proposed approach on 2 different tracks. %Pablo: repeated
Experimental results show that the evolutionary based fuzzy controller is competitive
with respect to the embedded TORCS controllers % comment new results Pablo: the objective should still be clarified, I still don't know what has been done in this paper by only reading the abstract
\end{abstract}
...
that matter, in a desert or hostile environment. However, this is only a
...
controllers, have made TORCS one of the most used simulator in the field of
...
publication.
One application of evolutionary algorithms was to determine the
...
% FUZZY + EAs
Our approach is focused in this line, proposing the application of an off-line genetic algorithm for the improvement of the parameters which determine the behavior of a controller for TORCS. We have focused on a Fuzzy-based model, as it is one of the best options for modeling human-like decisions and actions, as others authors have also used this kind of technique in the literature with good results \cite{torcs_Onieva2011}. 
For instance, in \cite{Guadarrama2008}, a fuzzy rule-based car controller for a Car Racing Competition was built and tuned with co-evolutionary genetic algorithms. Two fuzzy controllers were designed (acceleration and turning angle). %Pablo: in previous phrase you said one controller, not two
...
will be used in races against selected oponents. 
...
These results are presented in Table\ref{tab:gfc2real2} where one
could clearly notice the degradation of the performances of the
proposed  optimized fuzzy controller. Indeed, it was ranked in the
fourth position just before {\tt tita10} and {\ tt bt3}
controllers. Its top speed is acceptable considering that the used car
is not as fast as others but the fast lap time was higher  and the
GFC2 controller was not as competitive with its rivals in this race
parameter. These results could be easily justified by the nature of
the track. A-speedway track is an oval track with only four turns,
this kind of tracks deprives our controller of his strong point which
is the late braking and the selection of an optimal trajectory in
turns. 
The fact that the car has an % better? Worse? - JJ
average speed compared to the others and that our controller can not compensate for this loss of time in the turns will not work in favor of getting better best lap times.  
...
After initial tests, that showed the promise of using evolutionary algorithms with two different fitness functions, one 
...
(UGR-EPHEMECH) and  and TIN2017-85727-C4-2-P (UGR-DeepBio).
